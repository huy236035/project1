...
import math
# Không dùng NODES sample data nữa - chỉ dùng node_coordinates từ OSM

def haversine_distance(node1, node2, node_coordinates=None):
    """
    ...
    Args:
        node1: Node ID 1
        node2: Node ID 2
        node_coordinates: Dict chứa tọa độ các nodes (optional, để hỗ trợ OSM data)
    """
    # CHỈ lấy từ node_coordinates (OSM data) - không dùng sample data
    if node_coordinates:
        node1_str = str(node1)
        node2_str = str(node2)
        if node1_str in node_coordinates and node2_str in node_coordinates:
            coords1 = node_coordinates[node1_str]
            coords2 = node_coordinates[node2_str]
            lat1, lng1 = coords1.get('lat'), coords1.get('lng')
            lat2, lng2 = coords2.get('lat'), coords2.get('lng')
        else:
            # Không có tọa độ trong node_coordinates
            return 0
    else:
        # Không có node_coordinates - không thể tính heuristic
        return 0
...
def astar(graph, start_node, end_node, node_coordinates=None):
    """
    ...
    h_start = haversine_distance(start_node, end_node, node_coordinates)
    ...
    while open_set:
        ...
        for neighbor, weight in neighbors:
            ...
            h_neighbor = haversine_distance(neighbor, end_node, node_coordinates)
